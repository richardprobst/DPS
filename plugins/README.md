# Plugins do desi.pet by PRObst

Esta pasta contém todos os plugins do sistema desi.pet by PRObst: o plugin núcleo (base) e os add-ons complementares. Cada add-on é um plugin WordPress separado que **depende do plugin núcleo** (`desi-pet-shower-base`).
...
Cada subpasta neste diretório representa um plugin independente:

```
plugins/
├── desi-pet-shower-base/                # Plugin núcleo (OBRIGATÓRIO)
...
├── desi-pet-shower-groomers/            # Gerenciamento de tosadores
├── desi-pet-shower-loyalty/             # Programa de fidelidade
├── desi-pet-shower-payment/             # Integrações de pagamento
├── desi-pet-shower-push/                # Notificações push
├── desi-pet-shower-registration/        # Formulário de cadastro inicial
├── desi-pet-shower-services/            # Catálogo de serviços
├── desi-pet-shower-stats/               # Estatísticas e relatórios
├── desi-pet-shower-stock/               # Controle de estoque
└── desi-pet-shower-subscription/        # Gerenciamento de assinaturas
```

## Dependências

**TODOS os add-ons dependem do plugin base** (`desi-pet-shower-base`) para funcionar. O plugin núcleo fornece:
