...
trait DPS_Agenda_Renderer {
    ...
    public function render_appointment_row( $appt, $column_labels ) {
        ...
        echo '<td data-label="' . esc_attr( $column_labels['service'] ?? __( 'ServiÃ§o', 'dps-agenda-addon' ) ) . '">';
        ...
        echo '<td data-label="' . esc_attr( $column_labels['confirmation'] ?? __( 'ConfirmaÃ§Ã£o', 'dps-agenda-addon' ) ) . '">';
        ...
        echo '<td data-label="' . esc_attr( $column_labels['charge'] ?? __( 'CobranÃ§a', 'dps-agenda-addon' ) ) . '">';
    ...
    public function render_appointment_row_tab1_m3( $appt, $column_labels ) {
        ...
        echo '<td class="dps-agenda-cell dps-agenda-cell--service dps-col-service" data-label="' . esc_attr( $column_labels['service'] ?? __( 'ServiÃ§os', 'dps-agenda-addon' ) ) . '">';
        if ( is_array( $service_ids ) && ! empty( $service_ids ) ) {
        ...
        echo '<td class="dps-agenda-cell dps-agenda-cell--confirmation" data-label="' . esc_attr( $column_labels['confirmation'] ?? __( 'ConfirmaÃ§Ã£o', 'dps-agenda-addon' ) ) . '">';
    ...
    public function render_appointment_row_tab2_m3( $appt, $column_labels ) {
        ...
        echo '<td class="dps-agenda-cell dps-agenda-cell--status" data-label="' . esc_attr( $column_labels['status'] ?? __( 'Status do serviÃ§o', 'dps-agenda-addon' ) ) . '">';
        if ( is_user_logged_in() && current_user_can( 'manage_options' ) ) {
        ...
        if ( $has_checkout ) {
            $btn_icon  = 'âœ“';
            $btn_label = __( 'Ver operaÃ§Ã£o', 'dps-agenda-addon' );
            $btn_short = __( 'Ver operaÃ§Ã£o', 'dps-agenda-addon' );
            $btn_class = 'dps-expand-panels-btn--done';
        } elseif ( $has_checkin ) {
            $btn_icon  = 'â†’';
            $btn_label = __( 'Continuar operaÃ§Ã£o', 'dps-agenda-addon' );
            $btn_short = __( 'Continuar', 'dps-agenda-addon' );
            $btn_class = 'dps-expand-panels-btn--checkin';
        } else {
            $btn_icon  = '+';
            $btn_label = __( 'Abrir operaÃ§Ã£o', 'dps-agenda-addon' );
    ...
    public function render_appointment_row_tab3_m3( $appt, $column_labels ) {
        ...
        if ( $taxidog_requested ) {
            ...
            if ( ! empty( $client_address ) ) {
                ...
                echo '<a href="' . esc_url( $map_url ) . '" target="_blank" class="dps-taxidog-map-btn" title="' . esc_attr__( 'Abrir endereÃ§o no Google Maps', 'dps-agenda-addon' ) . '">' . esc_html__( 'Abrir mapa', 'dps-agenda-addon' ) . '</a>';
            }
        } else {
    ...
    public function render_appointment_row_tab1( $appt, $column_labels ) {
        ...
        echo '<td class="dps-col-service" data-label="' . esc_attr( $column_labels['service'] ?? __( 'ServiÃ§os', 'dps-agenda-addon' ) ) . '">';
        $service_ids = get_post_meta( $appt->ID, 'appointment_services', true );
        if ( is_array( $service_ids ) && ! empty( $service_ids ) ) {
    ...
    public function render_appointment_row_tab2( $appt, $column_labels ) {
        ...
        echo '<td data-label="' . esc_attr( $column_labels['status'] ?? __( 'Status do ServiÃ§o', 'dps-agenda-addon' ) ) . '">';
        $can_edit = is_user_logged_in() && current_user_can( 'manage_options' );
